...
| `004_beacon_case_paper.sql` | `beacon_cases.paper` — one additive `jsonb` column holding the Deep-Engagement paper artifact (`{status, engagement:{supporting,challenging,tensions}, draft:{...sections..., references[]}, generated_at}`) produced by `POST /api/beacon/paper` from an assembled case. Additive, idempotent, non-destructive; isolated from `CASE_COLS` so existing routes are unaffected. (research-027 / WFM-101) |
...
// paper — generated by POST /api/beacon/paper from an ASSEMBLED case (research-027). Reads the cased
// papers' FULL TEXT + runs ONE counter-search pass for disconfirming evidence. Refs E# = cased
// evidence, C# = counter-search hits; references[] is built server-side from sources actually read
// (no-fabrication). 028 renders it; 029 publishes a ratified version into the corpus.
paper: {
  "status": "draft",
  "engagement": {
