 ...
 *
 * `status` lifecycle:
 *   - `draft` — generated by the paper engine, member-private on the canvas.
 *   - `published_proposed` — the member published it into the corpus (research-029); a `papers`
 *     row now exists at status `proposed` (NOT live) awaiting curator/FOREMAN ratification.
 * `published_paper_id` traces the corpus `papers.id` created on publish so 028's UI can show the
 * publish state and link to it; `published_at` is when the member published (proposed, not ratified).
 */
export interface CasePaper {
  status: 'draft' | 'published_proposed'
  engagement: PaperEngagement
  draft: PaperDraft
  generated_at: string
  /** research-029: the corpus papers.id created when the member published this draft (proposed). */
  published_paper_id?: number
  /** research-029: ISO timestamp the member published the draft to the corpus (proposed). */
  published_at?: string
}
